...
/** Gera o conteúdo do PDF individual (sem salvar). Usado por generateIndividualPDF e getIndividualPDFBlobUrl. */
function buildIndividualPDF(doc: jsPDF, request: ExtraRequest) {
...
export const generateIndividualPDF = (request: ExtraRequest) => {
  const doc = new jsPDF();
  buildIndividualPDF(doc, request);
  doc.save(`solicitacao-${request.code}.pdf`);
};

/** Retorna URL do PDF individual para exibir em iframe (revogue com URL.revokeObjectURL quando não precisar mais). */
